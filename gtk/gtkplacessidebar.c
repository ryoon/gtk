  ...
  if (sidebar->drag_data_received &&
  ...
  else
    {
      ...
      if (context)
        ...
        {
          /* We cannot discern if it is valid or not because the drag source is
           * not above the sidebar widget, so we don't have dnd context, simply
           * make insensitive the drop targets we know are not valid for files,
           * and make sensitive all the others
           */
          valid = TRUE;
        }
    }
  ...
  if (visible)
    {
      ...
      start_drop_feedback (sidebar, NULL, NULL);
    }
  else
    {
      if (sidebar->drop_state == DROP_STATE_NEW_BOOKMARK_ARMED_PERMANENT ||
